...
module.exports = {
    ...
    async showSearchMenu(interaction, results, query, guildId) {
        ...
        // Create embed
        const E = (interaction.client && interaction.client.emoji) || {};
        const searchEmoji = E.youtube || '🎵';
        const embed = new EmbedBuilder()
            .setTitle(`${searchEmoji} ${searchTitle}`)
    ...
    // ✅ إضافة executePrefix (كان مفقوداً رغم وجود aliases)
    async executePrefix(message, args, client) {
        const query = args.join(' ').trim();
        if (!query) {
            return message.reply('❌ استخدم: `!search <اسم الأغنية>`');
        }
        ...
        const permissions = member.voice.channel.permissionsFor(guild.members.me);
        if (!permissions.has(PermissionFlagsBits.Connect) || !permissions.has(PermissionFlagsBits.Speak)) {
            return message.reply('❌ البوت يحتاج صلاحيات `Connect` و `Speak`.');
        }

        const statusMsg = await message.reply('🔍 جاري البحث...');

        try {
            const results = await YouTube.search(query, 9, guildId);
            if (!results || results.length === 0) {
                return statusMsg.edit('❌ لا توجد نتائج.');
            }

            // استدعِ showSearchMenu مع fake interaction يعمل عبر statusMsg
            ...
            await this.showSearchMenu(fakeInteraction, results, query, guildId);
        } catch (error) {
            console.error('❌ Search prefix error:', error.message);
            await statusMsg.edit('❌ حدث خطأ: ' + error.message).catch(() => {});
        }
    }
};